...
assert(mainActivity.includes("setImportantForAutofill"), "Android WebView autofill must be disabled for vault input")
assert(mainActivity.includes("setSaveEnabled(false)"), "Android WebView form saving must be disabled")
...
assert(appSource.includes("NO_IME_LEARNING_PROPS"), "Sensitive fields must disable IME learning/suggestions")
assert(appSource.includes("isAndroidNative || field.secret"), "Android credential values must use password input mode")
